...
def validate_model_structure(data: Dict[str, Any]) -> bool:
    ...
    required_fields = ["id", "title", "body", "created_at", "updated_at", "model_type"]
    if not all(field in data for field in required_fields):
        return False
    
    # Validate model_type structure
    return validate_model_type_structure(data["model_type"])
...
def validate_model_full_data_structure(data: Dict[str, Any]) -> bool:
    ...
    Expected structure:
    {
        "model": ModelDict (with model_type inside),
        "attributes": Dict[str, Any],
    ...
    required_fields = ["model", "attributes", "relations"]
    if not all(field in data for field in required_fields):
        return False
    
    # Validate model structure (which now includes model_type)
